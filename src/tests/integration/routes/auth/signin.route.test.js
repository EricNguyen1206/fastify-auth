...
// Import after mocks
const signinRoute = (await import('../../../../routes/auth/signin.route.js')).default;
...
describe('Signin Route - POST /auth/signin', () => {
  ...
  it('should successfully login with valid credentials', async () => {
    ...
    expect(mockAuthService.createAuthSession).toHaveBeenCalledWith(user, fastify.jwt);
  ...
  it('should set refresh token cookie', async () => {
    ...
    expect(reply.setCookie).toHaveBeenCalledWith('refreshToken', 'refresh-token-456', {
      httpOnly: true,
      secure: false,
      sameSite: 'strict',
      maxAge: 7 * 24 * 60 * 60,
  ...
  it('should return 401 for invalid email', async () => {
    ...
    expect(fastify.audit.security).toHaveBeenCalledWith('signin_failed', expect.objectContaining({
      email: 'wrong@example.com',
  ...
  it('should log successful signin', async () => {
    ...
    expect(fastify.log.info).toHaveBeenCalledWith('User logged in: test@example.com');
    expect(fastify.audit.auth).toHaveBeenCalledWith('signin_success', expect.objectContaining({
      userId: user.id,
      email: user.email,
    }));
  });
