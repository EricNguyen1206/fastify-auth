...
jest.unstable_mockModule('../../../repositories/session.repository.js', () => mockSessionRepository);
jest.unstable_mockModule('../../../configs/variables.js', () => ({ config: mockConfig }));

// Import service after mocks
const { signup, signin, createAuthSession, refreshAccessToken, signout } = await import('../../../services/auth.service.js');

// Import test helpers
import { createTestUser, createTestSession, createMockJWT } from '../../helpers/test-helpers.js';
...
describe('Auth Service - createAuthSession()', () => {
  ...
  it('should set correct token expiry (7 days)', async () => {
    ...
    expect(expiresAt.getTime()).toBeGreaterThanOrEqual(expectedExpiry.getTime());
...
describe('Auth Service - refreshAccessToken()', () => {
  ...
  it('should return new access token with valid refresh token', async () => {
    ...
    expect(mockSessionRepository.findSessionByToken).toHaveBeenCalledWith(refreshToken, 1);
    expect(mockJwt.sign).toHaveBeenCalledWith(
      { userId: 1 },
      { expiresIn: '15m' }
    );
  ...
  it('should throw 401 when token type is not refresh', async () => {
    ...
    await expect(refreshAccessToken(refreshToken, mockJwt)).rejects.toMatchObject({
...
describe('Auth Service - signout()', () => {
  ...
  it('should delete session when refresh token provided', async () => {
    ...
    expect(mockSessionRepository.deleteSession).toHaveBeenCalledWith(refreshToken);
  });
